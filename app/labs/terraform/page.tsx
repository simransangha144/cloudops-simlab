...
const resources: Resource[] = [
  ...
  { id: "manual-console", name: "Manual Console Changes", description: "Modify managed infrastructure outside Terraform", group: "operations", badge: "DRIFT" },
...
const ASSESSMENT_LOCK_ENABLED = false;
...
export default function TerraformLab() {
  ...
  const score = useMemo(() => {
    ...
    return { correctSelected, wrongSelected, missing };
  }, [selected]);

  const passed =
    score.correctSelected === correct.length && score.wrongSelected === 0;

  function toggle(id: ResourceId) {
    if (submitted || (ASSESSMENT_LOCK_ENABLED && typeof window !== "undefined" && localStorage.getItem("terraform-lab-submitted") === "1")) {
      return;
    }
  ...
  function submit() {
    if (submitted) return;
    setSubmitted(true);
    if (ASSESSMENT_LOCK_ENABLED && typeof window !== "undefined") {
      localStorage.setItem("terraform-lab-submitted", "1");
    }
  }
  ...
  function resetPractice() {
    ...
    if (typeof window !== "undefined") {
      localStorage.removeItem("terraform-lab-submitted");
    }
  }
        ...
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4">
          <div>
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              <span className="rounded-full border border-cyan-400/20 bg-cyan-400/[0.06] px-3 py-1 text-[9px] font-semibold uppercase tracking-wider text-cyan-300">
                LAB_04
              </span>
            </div>
          </div>
          ...
          <div className="hidden items-center gap-3 sm:flex">
            <span className="rounded-lg border border-white/10 bg-white/[0.02] px-3 py-2 font-mono text-[10px] text-gray-500">
              ADVANCED
            </span>
            <span className="rounded-lg border border-white/10 bg-white/[0.02] px-3 py-2 font-mono text-[10px] text-gray-500">
              25–35 MIN
            </span>
            <span className="rounded-lg border border-amber-400/20 bg-amber-400/[0.05] px-3 py-2 font-mono text-[10px] text-amber-300">
              ONE ATTEMPT
            </span>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-8">
          <div className="max-w-5xl">
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight sm:text-3xl">
              Stabilize a production Terraform platform
            </h2>
            <p className="mt-4 max-w-4xl text-sm leading-7 text-gray-400">
              You have inherited Terraform used to provision a customer-facing AWS
              application. The infrastructure works, but the repository has poor
              state management, weak environment isolation and unsafe operational
              practices. Select the components and practices that belong in a
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-[10px] text-gray-500">
                  Plausible options are intentionally mixed together.
                </p>
              </div>
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-[10px] text-gray-600">
                  Construct the production design before submitting.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                {!submitted && selected.length > 0 && (
                  <button
                    type="button"
                    onClick={() => setSelected([])}
                    className="rounded-lg border border-white/10 px-3 py-2 text-[10px] text-gray-500 hover:text-white"
                  >
                    Clear
                  </button>
                )}
                ...
                >
                  {submitted ? "Submitted" : "Submit Architecture"}
                </button>
              </div>
            </div>
            ...
            <div className="min-h-[720px] rounded-2xl border border-white/10 bg-[#070c14]/90 p-5">
              <div className="rounded-xl border border-cyan-400/10 bg-cyan-400/[0.015] p-5">
                ...
                <div className="mt-5 grid gap-4 xl:grid-cols-3">
                  <BoardSection title="STATE & WORKFLOW">
                    {selected
                      .filter((id) =>
                        ["s3-backend", "s3-versioning", "s3-lockfile", "dynamodb-lock", "local-backend", "auto-approve"].includes(id)
                      )
                  ...
                  <BoardSection title="REPOSITORY STRUCTURE">
                    {selected
                      .filter((id) =>
                        ["vpc-module", "network-module", "compute-module", "prod-environment", "dev-environment", "variables", "outputs", "data-source"].includes(id)
                      )
                      .map((id) => <SelectedCard key={id} resource={resources.find((r) => r.id === id)!} />)}
                  </BoardSection>
                  ...
                  <BoardSection title="AWS / SECURITY">
                    {selected
                      .filter((id) =>
                        ["alb", "ec2-asg", "rds", "nat", "secrets-manager", "iam-role", "security-group", "cloudwatch", "hardcoded-secret", "iam-admin", "public-s3", "manual-console", "null-resource", "terraform-taint"].includes(id)
                      )
                      .map((id) => <SelectedCard key={id} resource={resources.find((r) => r.id === id)!} />)}
                  </BoardSection>
                </div>
              ...
              {!submitted && (
                <div className="mt-5 rounded-xl border border-amber-400/10 bg-amber-400/[0.025] px-4 py-3 text-[10px] leading-5 text-gray-500">
                  ...
                  no correctness hints are shown while you build. Several resources are
                  deliberately plausible but inappropriate for this production scenario.
                </div>
              )}
              ...
              {submitted && (
                <ResultPanel
                  passed={passed}
                  correct={score.correctSelected}
                  wrong={score.wrongSelected}
...
function Requirement({ label }: { label: string }) {
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.015] px-3 py-3">
      <div className="flex items-center gap-2">
...
function FilterButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`whitespace-nowrap rounded-lg px-2.5 py-1.5 text-[8px] font-semibold tracking-wider transition ${
        active
          ? "bg-cyan-400/10 text-cyan-300"
...
function BoardSection({
  ...
  return (
    <div className="min-h-[250px] rounded-xl border border-white/[0.07] bg-black/15 p-3">
      <div className="mb-3 flex items-center justify-between">
        <span className="font-mono text-[8px] tracking-wider text-gray-600">{title}</span>
        <span className="h-1.5 w-1.5 rounded-full bg-white/10" />
      </div>
      <div className="space-y-2">
        {children || (
          <div className="flex min-h-[190px] items-center justify-center rounded-lg border border-dashed border-white/[0.06] text-[9px] text-gray-700">
            Select components
          </div>
        )}
      </div>
  ...
  return (
    <div className="rounded-lg border border-cyan-400/10 bg-cyan-400/[0.025] px-3 py-2.5">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[10px] font-medium text-gray-300">{resource.name}</span>
        <span className="font-mono text-[7px] text-cyan-400/70">{resource.badge}</span>
      </div>
      <p className="mt-1 text-[8px] leading-4 text-gray-600">{resource.description}</p>
    </div>
  );
}
...
function FlowCard({
  ...
  return (
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.015] p-4">
      <div className="font-mono text-[9px] text-cyan-400">{number}</div>
      <div className="mt-2 text-xs font-semibold">{title}</div>
      <div className="mt-1 text-[9px] leading-4 text-gray-600">{text}</div>
    </div>
  );
}
...
function ResultPanel({
  ...
  return (
    <div className="mt-5 rounded-2xl border border-white/10 bg-[#090f18] p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="font-mono text-[9px] uppercase tracking-widest text-gray-600">
            Assessment result
          </p>
          <h3 className={`mt-2 text-xl font-semibold ${passed ? "text-cyan-300" : "text-white"}`}>
            {passed ? "Architecture accepted" : "Architecture rejected"}
          </h3>
        </div>
        ...
        <div className={`rounded-xl border px-4 py-3 text-center ${
          ...
          <div className="font-mono text-xl font-bold">{correct}/{correct + missing}</div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <Metric label="Incorrect" value={String(wrong)} />
        <Metric label="Missing" value={String(missing)} />
      </div>

      {!passed && (
        <p className="mt-5 text-xs leading-6 text-gray-500">
          The assessment intentionally does not identify which individual selections
          were wrong. Review the scenario and Terraform operating model rather than
          relying on trial-and-error.
        </p>
      )}

      {!ASSESSMENT_LOCK_ENABLED && (
        <button
          type="button"
          onClick={resetPractice}
          className="mt-5 rounded-lg border border-white/10 px-4 py-2 text-[10px] text-gray-500 hover:text-white"
        >
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-white/[0.07] bg-white/[0.015] px-4 py-3">
      ...
      <div className="mt-1 text-[9px] uppercase tracking-wider text-gray-600">{label}</div>
    </div>
  );
}
